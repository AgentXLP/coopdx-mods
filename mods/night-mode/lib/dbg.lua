...
local function render_debug_points()
    ...
    for i = 1, _debug_points.count do
        ...
        if tmp_out1.z <= -260 then                   -- clip very near points
            ...
            djui_hud_set_color(0, 0, 0, c[4] and c[4]*0.8 or 204)
            djui_hud_render_rect(x - padding, y - padding, scale + padding * 2, scale + padding * 2)

            djui_hud_set_color(c[1], c[2], c[3], c[4] and c[4] or 255)
...
local function on_debug_hud_render_behind()
    clear_check()
    render_debug_points()
    render_debug_lines()
    render_debug_texts()
...
hook_event(HOOK_ON_HUD_RENDER_BEHIND, on_debug_hud_render_behind)
...
function Dbg.point(pos, color, scale)
    ...
    p.color[1], p.color[2], p.color[3], p.color[4] = color[1], color[2], color[3], color[4] or 255
    p.scale = scale
end
...
function Dbg.line(p1, p2, color, thickness)
    ...
    l.color[1], l.color[2], l.color[3], l.color[4] = color[1], color[2], color[3], color[4] or 255
...
function Dbg.text(string, pos, color, scale)
    ...
    t.string = string
    t.color[1], t.color[2], t.color[3], t.color[4] = color[1], color[2], color[3], color[4] or 255
    t.scale = scale
end
